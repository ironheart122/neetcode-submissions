class Solution {
  ...
  longestConsecutive(nums: number[]): number {
    // First attempt - 15/33 test cases passed
    ...
    for (const num of distinctNumsSet) {
      ...
      if (!distinctNumsSet.has(num - 1)) {
        ...
        // Keep traversing through the set while there's the next number in the set
        while (distinctNumsSet.has(current + 1)) {
          // if Found, increment the longest and then update current
          current++;
          length++;
        }
